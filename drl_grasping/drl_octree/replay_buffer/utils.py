...
def preprocess_stacked_pointcloud_batch(
    ...
    if n_aux_obs != 0:
        ...
        aux_obs = th.tensor(aux_obs, requires_grad=False).to(device)
    else:
        aux_obs = None

    # Convert to torch tensor
    points = observation[:, :, :-n_aux_obs, :]
    pointcloud_batch = th.tensor(points.reshape(-1, points.shape[2], points.shape[3]), requires_grad=False).to(device)
